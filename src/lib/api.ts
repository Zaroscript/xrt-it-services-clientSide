...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response?.status === 403) {
      const errorMessage = error.response?.data?.message || "You are not authorized to perform this action.";
      ...
      // Don't redirect for 403 errors, just show the error message
      // The component will handle displaying the error to the user
    }
